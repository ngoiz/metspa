...
PROGRAM_DIRECTORY = os.path.abspath(os.path.dirname(os.path.realpath(__file__)) + '/../')
...
class MeteoSpa:
    ...
    def __init__(self):
        ...
        parser.add_argument('function', help='Program function to run', choices=['fetch', 'clean'])

        self.number_of_arguments = 1  # in case new arguments are added

        options = parser.parse_args(sys.argv[1:self.number_of_arguments + 1])  # might need to change 2 to number of arguments if optionals included
    ...
    def fetch(self):
        ...
        parser.add_argument('-a', '--api', help='AEMET Open Data API Key or specifiy in config file')
        ...
        loader.extract_aemet_data(self.data,
                                  options.start_year,
                                  options.end_year,
                                  options.station_id
                                  )
    ...
    def clean(self):
        ...
        folders_to_delete = [os.path.join(self.data['program_directory'], 'temp_output/')
                             ]
...
def get_output_directory(options):
    ...
    if options.output:
        ...
        try:
            ...
            logging.warning(f'Output directory ``output_directory`` not set in config file, reverting to default.')
