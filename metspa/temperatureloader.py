...
def load_api(API_KEY):
    ...
    api_instance = swagger_client.ValoresClimatologicosApi(
...
def extract_aemet_data(data, start_year, end_year, idema):
    ...
    for year_ini in range(start_year, end_year + 1, 4):
        ...
        datafile = (
            temp_json_data_directory + f"/climat{idema}_S{year_ini}_E{year_last}.json"
        )

        if os.path.exists(datafile):
            logging.info(
                "AEMET Data found for current period. To refresh clear temp"
                "directory with `metspa clean`."
            )
            continue

        try:
            api_response = api_instance.climatologas_diarias_(
                start_date, end_date, idema
            )
    ...
    processdata.clean_data_from_multiple_json(
        temp_json_data_directory, data["output_directory"], idema
    )
